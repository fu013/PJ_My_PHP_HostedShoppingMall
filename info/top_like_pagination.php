<?php
    ...
    $product_select = "select product_autoNum, p.product_no, product_category, product_name, product_price, product_size, product_color, product_view, product_like, im.main_img_dir_name, p.created_at FROM product p inner join images_main im on p.product_no = im.product_no order by product_like desc".' '.$sqlLimit; // 프로덕트에 대한정보 + 해당 프로덕트넘버와 같은 프로덕트넘버를 가진 메인이미지네임을 가져옴.
