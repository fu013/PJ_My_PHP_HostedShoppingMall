<?php
    ...
    $product_miyaong = "select count(*) count from seungchanshop25.product";
    ...
    $product_select = "select product_autoNum, p.product_no, product_category, product_name, product_price, 
    product_size, product_color, product_view, product_like, im.main_img_dir_name, p.created_at FROM product p inner join images_main im on p.product_no = im.product_no where p.product_category = '$category'
    order by created_at desc".' '.$sqlLimit; // 프로덕트에 대한정보 + 해당 프로덕트넘버와 같은 프로덕트넘버를 가진 메인이미지네임을 가져옴.
    // 이부분에서 티셔츠면 티셔츠카테고리그렇게만 분류해주면끝
