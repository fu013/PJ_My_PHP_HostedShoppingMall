...
<html >
  <head>
   ...
   <script type="text/javascript">
    // 체크박스 모두 동의 여부 확인 스크립트
    function agree() { 
                var chkbox = document.getElementsByName('agree');
                // 배열형식으로 가져옴 => NodeList(5) [input, input, input, input, input]
                var chk = true; 
                for(var i=0 ; i<chkbox.length ; i++) { 
                    if(chkbox[i].checked) { 
                        ...
                        chk = false;
                        // 포문이 첫인덱스부터 도는데 중간에 한 체크박스라도 체크가 되어있지않다면 false를 리턴함, 즉 모든 체크박스가 다 true여야 스무스하게 포문이 돌면서 true가 리턴이됨
                        // 하나라도 false라면 그 즉시 포문이 멈추고 false를 리턴하기때문에 , 절대로 하나라도 false일시 true가 나올수가없음 break 활용 ㅆㅅㅌㅊ
                        break; 
                    }
                }
                
                if(chk === true ) { 
                    window.location.assign("sign_up_2.php"); // 다음 챕터로 이동
                } else { 
                    alert("모든 약관에 동의하세요!")
                    // 페이지 그대로 유지 (아무일도 일어나지않고 경고창만 띄워줌.)
                } 
            }
  </script>
  </head>
...
<section class="agreement_wrapper">
  <table width=960px align=center>
    <tr>
      <td></td>
      <h1 style="text-align: center; font-size: 2.0rem; font-weight: 300;">이용약관</h1>
      <ul class="agreement_photo">
                   <li><i class="xi-document" style="opacity: 1.0;"></i></li>
...
제24조(재판권 및 준거법)
 ...
 ②“몰”과 이용자간에 제기된 전자상거래 소송에는 한국법을 적용합니다.
            ...
            <tr>
              <td>
                <form action="">
                <input type="checkbox" name="agree">이용약관에 동의합니다.<br><br><br><br>
              </td>
            </tr>
            <tr>
              <td>
...
'정보통신망 이용촉진 및 정보보호 등에 관한 법률'의 거래기록 보존 의무에 따른 보존: 1년
            ...
            <tr>
              <td>
                <input type="checkbox" name="agree">개인정보 수집 및 이용에 대한 안내에 동의합니다.
              </form>
              </td>
            </tr>
          </table>
...
<br><br>
   <button type = "button" onclick="agree()"  class="btn_order" style="margin:10px;">확 인 하 기</button>
  <a href="index.php"> <button type = "button"  class="btn_cancel1" style="margin:10px;">취 소 하 기</button></a>
    </center>
  </section>
