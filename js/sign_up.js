...
window.onload = function() { 
  ...
  const register = document.getElementById('register'); // 찾은 서브밋타입 버튼
  register.addEventListener('click', function(){ // 클릭했을때 onload + 클릭했을때니깐 클릭하기전에 모든걸 한번더 읽었을경우라는 전제가 포함된다.
    ...
    let user_phone2 = document.getElementById('user_phone2').value;
    let user_phone3 = document.getElementById('user_phone3').value;
    let user_grade = document.getElementById('grade').value;
    ...
    let user_data_json = {
      ...
      "user_birth":user_birth, "user_gender":user_gender, "user_email":user_email, "user_phone1":user_phone1,
      "user_phone2":user_phone2, "user_phone3":user_phone3, "user_grade":user_grade
    }; 

    $.ajax({
        type: "post",
        data: user_data_json,
        url: "action/signUp_data.php",
        dataType: "html",
        success: function(data) {
          location.href='sign_up_3.php';
        }
    });
  })
}
